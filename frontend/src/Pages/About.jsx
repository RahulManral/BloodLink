...
const AboutUsCard = ({ title, description, image }) => {
  return (
    <motion.div 
      className="flex flex-col items-center bg-red-50 shadow-lg rounded-lg border-4 border-black h-96 font-customFont max-w-xs w-full transform transition-transform duration-300 hover:-translate-y-2"
  ...
  return (
    <section className="bg-red-50 bg-fixed bg-cover bg-center px-3 xs:pt-4 ms:pt-4 lg:pt-0 xl:pt-0" style={{ backgroundImage: `url(${backgroundImage})` }}>
      <div className="bg-transparent xs:bg-auto lg:p-8 xl:p-8 max-w-screen-xl mx-auto">
        <motion.div 
          className="flex flex-col md:flex-row items-center p-6 bg-red-200 rounded-xl border-4 border-black"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
